...
c_is_user_admin::c_is_user_admin() {
	...
	*/
	...
	if (!b) throw std::runtime_error("Administrator permissions required");
}
...
{
	assert(m_stream_handle.is_open());
}
...
void c_tuntap_windows_obj::async_receive_from_tun(unsigned char *const data, size_t size, const read_handler &handler) {
	//using read_handler = std::function<void(const unsigned char *, std::size_t, const boost::system::error_code &)>;
	static std::array<unsigned char, 14 + 40 + 65535> input_buffer; // !!! multithread unsafe
	m_stream_handle.async_read_some(boost::asio::buffer(input_buffer),
		[data, handler, this](const boost::system::error_code& error, std::size_t size) {
			if (c_ndp::is_packet_neighbor_solicitation(input_buffer.data(), size)) {
				const std::array<unsigned char, 94> neighbor_advertisement_packet_array = c_ndp::generate_neighbor_advertisement_new(m_mac_address, input_buffer);
				m_stream_handle.write_some(boost::asio::buffer(neighbor_advertisement_packet_array));
				handler(data, 0, error);
			}
			std::copy_n(input_buffer.begin() + 14, size - 14, data); // without eth header
			handler(data, size, error);
		} // lambda
	);
}
...
void c_tuntap_windows_obj::set_tun_parameters(const std::array<unsigned char, IPV6_LEN> &binary_address, int prefix_len, uint32_t mtu) {
	...
	assert(sizeof(iprow.Address.Ipv6.sin6_addr) == binary_address.size()); // static_assert not works on MSVC
...
void c_tuntap_windows_obj::set_mtu(uint32_t mtu) {
	...
	LONG status = 1;
	HKEY key = nullptr;
	assert(!m_register_tun_path.empty());
